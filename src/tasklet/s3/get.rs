...
/// let _tasklet = S3GetTaskletBuilder::new()
///     .bucket("my-bucket")
///     .key("file.csv")
...
impl S3GetTaskletBuilder {
    ...
    /// Sets the local file path to download to.
    ...
    pub fn build(self) -> Result<S3GetTasklet, BatchError> {
        ...
        Ok(S3GetTasklet { config: self.config, bucket, key, local_file })
    }
}

impl Default for S3GetTaskletBuilder {
    fn default() -> Self {
        Self::new()
    }
}

/// Tasklet that downloads an S3 object to a local file. (Placeholder — full implementation in Task 5.)
#[allow(dead_code)]
